...
import FormInvitationService from "./FormInvitationService";
const InvitationServicePage = (props) => {
  const {
    invitationService,
    invitationServices,
    isLoading,
  ...
  const [items, setItems] = useState(invitationServices || []);
  ...
  const handleDelete = () => {
    deleteInvitationService(formState?.id);
  };
  const handleAdd = () => {
    addInvitationService(formState);
  };
  const handleUpdate = () => {
    updateInvitationService(formState?.id, formState);
  };
  useEffect(() => {
    fetchInvitationServices(perPage, currentPage);
  }, [perPage, currentPage]);
  useEffect(() => {
    if (invitationServices) {
      const filteredItems = invitationServices?.filter((item) => {
        const isTextMatch =
      ...
      <div class="page-header d-print-none">
        <div class="container-xl">
          <div class="row g-2 align-items-center">
            <div class="col">
              <div class="page-pretitle">Master Layanan Undangan</div>
              <h2 class="page-title">Layanan Undangan</h2>
            </div>
                  ...
                  <div class="card-header ">
                    <div class="row w-full">
                      <div class="col-md-9 col-12">
                        <h3 class="card-title mb-0">Layanan Undangan</h3>
                        <p class="text-secondary m-0">
                          Master Layanan Undangan di justforyou
                        </p>
                      </div>
                      ...
                      <table class="table table-vcenter table-selectable">
                        ...
                        <tbody className="table-tbody">
                          {isLoading
                            ...
                            : paginatedItems?.map(
                                (invitationService, index) => (
                                  <tr key={invitationService?.id}>
                                      ...
                                      <div class="btn-list flex-nowrap justify-content-center">
                                        <a
                                          ...
                                          onClick={() => {
                                            setIsEdit(true);
                                            setFormState({
                                              id: invitationService?.id,
                                              name: invitationService?.name,
                                              detail: invitationService?.detail,
                                            });
                                            setIsShowModal(true);
                                          }}
                                        >
...
const mapStateToProps = (state) => ({
  ...
  total_data: state?.invitationServices?.datas?.total,
  isLoading: state?.invitationServices?.loading,
});
